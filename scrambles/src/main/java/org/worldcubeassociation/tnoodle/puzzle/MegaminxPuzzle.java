...
public class MegaminxPuzzle extends Puzzle {
	...
	private static void swapOnSide(int[][] image, int b, int f1, int s1, int f2, int s2, int f3, int s3, int f4, int s4,
			int f5, int s5) {
		for (int i = 0; i < 3; i++) {
	...
	private static Path pentagon(boolean pointup, int minxRad) {
		...
		p.lineTo(x[0], y[0]); // TODO - this is retarded, why do i need to do this? it would appear that
								// closePath() isn't doing it's job
		p.closePath();
		return p;
	}

	private static Point2D.Double getLineIntersection(double x1, double y1, double x2, double y2, double x3, double y3,
			double x4, double y4) {
	...
	private boolean isNormalized(int[][] image) {
		return image[Face.U.ordinal()][centerIndex] == Face.U.ordinal()
				&& image[Face.F.ordinal()][centerIndex] == Face.F.ordinal();
	}
	...
	class MegaminxState extends PuzzleState {
		...
		private void drawPentagon(Svg g, Path p, int[] state, int rotateCounterClockwise, String label,
			...
			for (int i = 0; i < intpent.length; i++) {
				intpent[i] = getLineIntersection(xs[i], ys[i], xs[5 + (3 + i) % 5], ys[5 + (3 + i) % 5],
